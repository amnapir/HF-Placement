...
We'll employ the Leave-One-Out Cross-Validation (LOO) technique to generate equivalent null models with standard priors. Subsequently, we'll utilize the same dataset across all models to evaluate them from an information-theoretic standpoint. We have demonstared it for "Effectivness" here, but using the same script you can do it for other variables. We found the same resulst for all other variables.
